# MCClauneck Extension Template

A starter template for creating extensions for the [MCEconomy](https://github.com/MCEngine/mceconomy) plugin. This repository provides the boilerplate code, Gradle configuration, and lifecycle management required to build add-ons using the [MCExtension API](https://github.com/MCEngine/mcextension).
...
1. Rename the package directory: `src/main/java/io/github/mcclauneck/template` -> `src/main/java/io/github/yourname/yourextension`.
...
Because this project depends on `mcextension` and `mceconomy-common` hosted on GitHub Packages, you must authenticate.
...
**Local Development (Auto-Deploy):** If you want the jar to be automatically copied to a test server's plugin folder after building, set the `MCEXTENSION_MCECONOMY_BUILD_PATH` environment variable.

* Linux/Mac: `export MCEXTENSION_MCBACKPACK_BUILD_PATH="/path/to/server/plugins/MCEconomy/extensions"`
* Windows: `set MCEXTENSION_MCBACKPACK_BUILD_PATH="C:\path\to\server\plugins\MCEconomy\extensions"`
...
* MCEconomy Repo: https://github.com/MCEngine/mceconomy
* MCExtension Libs: https://github.com/MCEngine/mcextension

<hr>

*Maintained by MCClauneck*
